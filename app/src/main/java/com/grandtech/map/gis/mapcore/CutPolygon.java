...
public class CutPolygon {
    ...
    {
        int gonCount=gon.getPointCount();
        int lineCount=line.getPointCount();
        ...
        //这种循环会漏掉多边形的一条边（第一个顶点和最后一个顶点构成的边）
        for (int i = 0; i < gon.getPointCount() - 1; i++){
            for (int j = 0; j < line.getPointCount() - 1; j++){
                System.out.println(">>>>>>>:("+i+","+(i+1)+");("+j+","+(j+1)+")");
                Point _intersectP = CutPolygon.SegmentInterect(gon.getPoint(i), gon.getPoint(i + 1), line.getPoint(j),  line.getPoint(j+1));//计算两线段交点
                if (_intersectP != null){
                    if (gon_index1 == -1 && line_index1 == -1 && intersectP1 == null){ //第一个交点
                        gon_index1 = i;
                        line_index1 = j;
                        intersectP1 = _intersectP;
                    }else if (gon_index2 == -1 && line_index2 == -1 && intersectP2 == null){ //第二个交点
                        gon_index2 = i;
                        line_index2 = j;
            ...
            if(i==gon.getPointCount() - 2){
                for (int j = 0; j < line.getPointCount() - 1; j++){
                    System.out.println(">>>>>>>:(0,"+(i+1)+");("+j+","+(j+1)+")");
                    Point _intersectP = CutPolygon.SegmentInterect(gon.getPoint(i + 1),gon.getPoint(0),  line.getPoint(j),  line.getPoint(j+1));//计算两线段交点
                    if (_intersectP != null){
                        if (gon_index1 == -1 && line_index1 == -1 && intersectP1 == null){ //第一个交点
                            gon_index1 = i + 1;
                            line_index1 = j;
